...
def main():
    ...
    pretrained_cfg = json.load(open(os.path.join(model_save_dir, f"{cfg['pretrained_model']}.json"), 'r'))
    with open(os.path.join(pretrained_cfg['data_path'], pretrained_cfg['charset_fname']), 'rb') as f:
    ...
    print('pretrained: '+ cfg['pretrained_model'])
    pretrained_model_state = os.path.join(model_save_dir, f"{cfg['pretrained_model']}_best.pt")
    pretrained_net = torch.load(pretrained_model_state)
